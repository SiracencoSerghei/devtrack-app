<script lang="ts">
	...
	async function handleSignUp(e: SubmitEvent) {
		...
		try {
			...
			successMsg = i18n.t('signup_page.success_msg') || 'Registrazione completata!';
...
<div class="auth-container">
	...
	<form onsubmit={handleSignUp}>
		<div class="form-group">
			...
			<input type="text" id="name" bind:value={name} disabled={loading} required placeholder={i18n.t('signup_page.name_placeholder')} />
		</div>
		...
		<button type="submit" class="btn-submit" disabled={loading}>
			{loading ? 'Loading...' : i18n.t('signup_page.btn_submit')}
		</button>
	</form>
...
<style>
	...
	label { margin-bottom: 0.4rem; font-weight: 500; color: #475569; font-size: 0.875rem; }
	input { padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 1rem; color: #1e293b; }
	input:focus { border-color: #2563eb; outline: none; box-shadow: 0 0 0 2px rgba(37,99,235,0.1); }
	input:disabled { background: #f8fafc; color: #94a3b8; }
	.btn-submit { width: 100%; padding: 0.7rem; background: #2563eb; color: white; border: none; border-radius: 6px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: background 0.2s; }
	.btn-submit:hover:not(:disabled) { background: #1d4ed8; }
	.btn-submit:disabled { background: #94a3b8; cursor: not-allowed; }
	.alert { padding: 0.8rem; margin-bottom: 1rem; border-radius: 6px; font-weight: 500; font-size: 0.875rem; }
